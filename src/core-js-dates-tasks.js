...
function getNextFriday(date) {
  ...
  const nextFriday = weekDay === 5 ? 7 : (5 - weekDay + 7) % 7;
  today.setDate(today.getDate() + nextFriday);
  return today;
}
...
function getCountDaysInMonth(month, year) {
  ...
  const endMonth = monthNumber + 1 < 12 ? monthNumber + 1 : 0;
  const endYear = monthNumber + 1 > 11 ? year + 1 : year;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timeDif = endDay.getTime() - startDay.getTime();
...
function isDateInPeriod(date, period) {
  ...
  return curDay >= startPeriod && curDay <= endPeriod;
}
...
function formatDate(date) {
  ...
  const clockMin = minutes > 10 ? minutes : `0${minutes}`;
  const clocksec = sec > 10 ? sec : `0${sec}`;

  return `${month}/${day}/${year}, ${clock}:${clockMin}:${clocksec} ${amPm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    const currentDte = new Date(year, month - 1, day);
    if (currentDte.getDay() === 6 || currentDte.getDay() === 0) {
      weekendsCount += 1;
    }
  }
